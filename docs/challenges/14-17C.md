# 15-19C: Challenges from Chapters 15 to 19
...
While it is stated that `getLine()` doesn't need to be efficient, `O(N)` time complexity isn't great for getting line information lmao
...
This has not been implemented as of writing. A variant where `LONG` variants take two-byte operands could be considered for extending the number of constants to 65536 as opposed to 16777216 for a three-byte operand.
...
**Marked as TODO.** As described above in 17-2. The bytecode emitted is similar to that of an if-then-else control flow.