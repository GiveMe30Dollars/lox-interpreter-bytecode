...
It should be noted that not only `OP_CONSTANT` requires an additional opcode; *anything* that uses a constants index requires a corresponding `LONG` variant. This includes the opcodes for interacting with global variables: `OP_DEFINE_GLOBAL`, `OP_GET_GLOBAL`, `OP_SET_GLOBAL`, as well as `OP_CLOSURE`. Then, those that interact with classes and instance getters, setters and invocations: `OP_CLASS` `OP_GET_PROPERTY`, `OP_SET_PROPERTY`, `OP_INVOKE`, `OP_GET_SUPER`, `OP_SUPER_INVOKE`. This increases the number of opcode variants to 11.
